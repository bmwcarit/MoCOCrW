...
protected:
    ...
    std::unique_ptr<RSAPadding> sha256PKCSPadding;
    std::unique_ptr<RSAPadding> sha512PKCSPadding;
    std::unique_ptr<RSAPadding> sha256PSSPadding;
    std::unique_ptr<RSAPadding> sha512PSSPadding;

    /* Build for the crypto context objects*/
...
void SignatureTest::SetUp()
{
    ...
    sha256PKCSPadding = std::make_unique<PKCSPadding>(DigestTypes::SHA256);
    sha512PKCSPadding = std::make_unique<PKCSPadding>(DigestTypes::SHA512);
    sha256PSSPadding = std::make_unique<PSSPadding>(DigestTypes::SHA256);
    sha512PSSPadding = std::make_unique<PSSPadding>(DigestTypes::SHA512);
}
...
TEST_F(SignatureTest, testSuccessfulRsaSignatureOfMessageDigestPKCS1ComparedToKnownOutput)
{
    ...
    auto ctx256 = builder.rsaPaddingMode(sha256PKCSPadding).build(key);
    EXPECT_NO_THROW(signature = ctx256.sign(_testMessageDigestSHA256));
    EXPECT_EQ(_validPKCS1SignatureSHA256, signature);

    auto ctx512 = builder.rsaPaddingMode(sha512PKCSPadding).build(key);
    EXPECT_NO_THROW(signature = ctx512.sign(_testMessageDigestSHA512));
...
{
    std::vector<uint8_t> signature;
    AsymmetricPublicKey pubKey = *_keyPairRsa;

    /* Sign and verify with SHA256 hashing */
    auto ctxPkcs256 = builder.rsaPaddingMode(sha256PKCSPadding).build(*_keyPairRsa);//{sha256PKCSPadding, *_keyPairRsa};
    EXPECT_NO_THROW(signature = ctxPkcs256.sign(_testMessageDigestSHA256));
    AsymmetricPubkeyCtx ctxVerifyPkcs256 = builder.rsaPaddingMode(sha256PKCSPadding).build(pubKey);//{sha256PKCSPadding, *_keyPairRsa};
    EXPECT_NO_THROW(ctxVerifyPkcs256.verify(signature, _testMessageDigestSHA256));//SignatureUtils::verify(ctxVerifyPkcs256, signature, _testMessageDigestSHA256));

    /* Sign and verify with SHA512 hashing */
    AsymmetricPrivkeyCtx ctxPkcs512 = builder.rsaPaddingMode(sha512PKCSPadding).build(*_keyPairRsa);
    EXPECT_NO_THROW(signature = ctxPkcs512.sign(_testMessageDigestSHA512));
    AsymmetricPubkeyCtx ctxVerifyPkcs512 = builder.rsaPaddingMode(sha512PKCSPadding).build(pubKey);//{sha512PKCSPadding, *_keyPairRsa};
    EXPECT_NO_THROW(ctxVerifyPkcs512.verify(signature, _testMessageDigestSHA512));
}
...
TEST_F(SignatureTest, testSuccessfulRsaSignatureAndVerificationPSSWithSHA256Masking)
{
    ...
    /* Sign and verify with SHA256 hashing */
    AsymmetricPrivkeyCtx ctxPss256 = builder.rsaPaddingMode(sha256PSSPadding).build(*_keyPairRsa);
    EXPECT_NO_THROW(signature = ctxPss256.sign(_testMessageDigestSHA256));
    AsymmetricPubkeyCtx ctxVerifyPss256 = builder.rsaPaddingMode(sha256PSSPadding).build(pubKey);
    EXPECT_NO_THROW(ctxVerifyPss256.verify(signature, _testMessageDigestSHA256));

    /* Sign and verify with SHA512 hashing */
    AsymmetricPrivkeyCtx ctxPss512 = builder.rsaPaddingMode(sha512PSSPadding).build(*_keyPairRsa);
    EXPECT_NO_THROW(signature = ctxPss512.sign(_testMessageDigestSHA512));
    AsymmetricPubkeyCtx ctxVerifyPss512 = builder.rsaPaddingMode(sha512PSSPadding).build(pubKey);
    EXPECT_NO_THROW(ctxVerifyPss512.verify(signature, _testMessageDigestSHA512));
}
...
TEST_F(SignatureTest, testSuccessfulRsaSignatureAndVerificationPSSWithSHA512Masking)
{
    ...
    std::unique_ptr<RSAPadding> padding = std::make_unique<PSSPadding>(DigestTypes::SHA512, DigestTypes::SHA512);
    AsymmetricPrivkeyCtx ctxPss512 =  builder.rsaPaddingMode(padding).build(*_keyPairRsa);

    EXPECT_NO_THROW(signature = ctxPss512.sign(_testMessageDigestSHA512));

    AsymmetricPubkeyCtx ctxVerifyPss512 = builder.rsaPaddingMode(padding).build(pubKey);
    EXPECT_NO_THROW(ctxVerifyPss512.verify(signature, _testMessageDigestSHA512));
}
...
TEST_F(SignatureTest, testSuccessfulRsaSignatureAndVerificationPSSWithNonDefaultSaltLength)
{
    std::vector<uint8_t> signature;
    AsymmetricPublicKey pubKey = *_keyPairRsa;
    std::unique_ptr<RSAPadding>  padding = std::make_unique<PSSPadding>(DigestTypes::SHA512,
                                                                        DigestTypes::SHA512, 13);

    AsymmetricPrivkeyCtx ctxPss512 = builder.rsaPaddingMode(padding).build(*_keyPairRsa);//{padding, *_keyPairRsa};
    EXPECT_NO_THROW(signature = ctxPss512.sign(_testMessageDigestSHA512));
...
TEST_F(SignatureTest, testUnsuccessfulRsaSignatureWithInvalidDigestLengthPKCS1)
{
    ...
    AsymmetricPrivkeyCtx ctxPkcs512 = builder.rsaPaddingMode(sha512PKCSPadding).build(*_keyPairRsa);
    EXPECT_THROW(signature = ctxPkcs512.sign(messageDigest), MoCOCrWException);
}
...
{
    std::unique_ptr<RSAPadding> padding = std::make_unique<PSSPadding>(DigestTypes::SHA256,
                                                                       DigestTypes::SHA256, -200);

    AsymmetricPrivkeyCtx ctxPss512 = builder.rsaPaddingMode(padding).build(*_keyPairRsa);
    /* Invalid case */
    ASSERT_THROW(ctxPss512.sign(_testMessageDigestSHA256), MoCOCrWException);
}
...
TEST_F(SignatureTest, testSuccessfulRsaVerificationWithValidPublicKey)
{
    ...
    AsymmetricPubkeyCtx ctxPkcs256 = builder.rsaPaddingMode(sha256PKCSPadding).build(key);

    EXPECT_NO_THROW(ctxPkcs256.verify(_validPKCS1SignatureSHA256, _testMessageDigestSHA256));
}
...
TEST_F(SignatureTest, testSuccessfulRsaVerificationWithCertificate)
{
    ...
    AsymmetricPubkeyCtx ctxPkcs256 = builder.rsaPaddingMode(sha256PKCSPadding).build(*_root1_RsaCert);
...
TEST_F(SignatureTest, testUnsuccessfulRsaVerificationWithWrongPublicKey)
{
    ...
    AsymmetricPrivkeyCtx ctxPss256 = builder.rsaPaddingMode(sha256PSSPadding).build(*_keyPairRsa);//{sha256PSSPadding, *_keyPairRsa};
    EXPECT_NO_THROW(signature = ctxPss256.sign(_testMessageDigestSHA256));

    /* Load Public Key file */
    _root1_RsaPubkey = std::make_unique<AsymmetricPublicKey>(loadPubkeyFromFile("root1.pubkey.pem"));
    AsymmetricPubkeyCtx ctxPssVerify256 = builder.rsaPaddingMode(sha256PSSPadding)
                                                 .build(*_root1_RsaPubkey);
    /* Using different public key */
    ASSERT_THROW(ctxPssVerify256.verify(signature, _testMessageDigestSHA256), MoCOCrWException);
}
...
TEST_F(SignatureTest, testUnsuccessfulRsaVerificationWithWrongPadding)
{
    ...
    AsymmetricPubkeyCtx ctxPkcsVerify256 = builder.rsaPaddingMode(sha256PKCSPadding).build(pubKey);
    ASSERT_THROW(ctxPkcsVerify256.verify(signature, _testMessageDigestSHA256),
                 MoCOCrWException);
}
...
TEST_F(SignatureTest, testUnsuccessfulRsaVerificationWithWrongHashingFunction)
{
    ...
    AsymmetricPubkeyCtx ctxPssVerify512 = builder.rsaPaddingMode(sha512PSSPadding).build(pubKey);
    ASSERT_THROW(ctxPssVerify512.verify(signature, _testMessageDigestSHA256),
                 MoCOCrWException);
}
...
TEST_F(SignatureTest, testUnsuccessfulRsaVerificationWithWrongMaskingFunction)
{
    ...
    std::unique_ptr<RSAPadding> padding = std::make_unique<PSSPadding>(DigestTypes::SHA256,
                                                                       DigestTypes::SHA512);
    AsymmetricPubkeyCtx ctxPssVerify256_512 = builder.rsaPaddingMode(padding).build(pubKey);
    ASSERT_THROW(ctxPssVerify256_512.verify(signature, _testMessageDigestSHA256), MoCOCrWException);
}
...
TEST_F(SignatureTest, testUnsuccessfulRsaVerificationWithWrongSaltLength)
{
    ...
    std::unique_ptr<RSAPadding> padding = std::make_unique<PSSPadding>(DigestTypes::SHA256,
                                                                       DigestTypes::SHA256, 2);
    AsymmetricPubkeyCtx ctxPssVerify256 = builder.rsaPaddingMode(padding).build(pubKey);
    ASSERT_THROW(ctxPssVerify256.verify(signature, _testMessageDigestSHA256), MoCOCrWException);
}
...
{
    std::vector<uint8_t> signature;
    AsymmetricPublicKey pubKey = *_keyPairEcc;

    /* Sign and verify with SHA1 hashing */
    AsymmetricPrivkeyCtx ctxEcc1 = builder.eccMaskingFunction(DigestTypes::SHA1).build(*_keyPairEcc);
    EXPECT_NO_THROW(signature = ctxEcc1.sign(_testMessageDigestSHA1));
    AsymmetricPubkeyCtx ctxEccVerify1 = builder.eccMaskingFunction(DigestTypes::SHA1).build(pubKey);
    EXPECT_NO_THROW(ctxEccVerify1.verify(signature, _testMessageDigestSHA1));

    /* Sign and verify with SHA256 hashing */
    AsymmetricPrivkeyCtx ctxEcc256 = builder.eccMaskingFunction(DigestTypes::SHA256).build(*_keyPairEcc);
    EXPECT_NO_THROW(signature = ctxEcc256.sign(_testMessageDigestSHA256));
    AsymmetricPubkeyCtx ctxEcc256Verify = builder.eccMaskingFunction(DigestTypes::SHA256).build(pubKey);
    EXPECT_NO_THROW(ctxEcc256Verify.verify(signature, _testMessageDigestSHA256));

    /* Sign and verify with SHA512 hashing */
    AsymmetricPrivkeyCtx ctxEcc512 = builder.eccMaskingFunction(DigestTypes::SHA512).build(*_keyPairEcc);
    EXPECT_NO_THROW(signature = ctxEcc512.sign(_testMessageDigestSHA512));
    AsymmetricPubkeyCtx ctxEccVerify512 = builder.eccMaskingFunction(DigestTypes::SHA512).build(pubKey);
    EXPECT_NO_THROW(ctxEccVerify512.verify(signature, _testMessageDigestSHA512));
}
...
TEST_F(SignatureTest, testSuccessfulEccVerificationWithValidEccPublicKey)
{
    ...
    AsymmetricPubkeyCtx ctxEccVerify1 = builder.eccMaskingFunction(DigestTypes::SHA1).build(publicKey);
    EXPECT_NO_THROW(ctxEccVerify1.verify(_validEccSignatureSHA1, _testMessageDigestSHA1));
}
...
TEST_F(SignatureTest, testUnsuccessfulEccVerificationWithInvalidSignature)
{
    ...
    /* The signature is modified and should now be invalid. */
    std::random_shuffle(signature.begin(), signature.end());
    ASSERT_THROW(ctxEccVerify1.verify(signature, _testMessageDigestSHA1),
                 MoCOCrWException);
}
...
TEST_F(SignatureTest, testSuccessfulEccVerificationWithCertificate)
{
    ...
    /* Verify with message digest */
    AsymmetricPubkeyCtx ctxEccVerify1 = builder.eccMaskingFunction(DigestTypes::SHA1)
                                               .build(*_root1_RsaCert);
    EXPECT_NO_THROW(ctxEccVerify1.verify(_validEccSignatureSHA1, _testMessageDigestSHA1));
}
...
TEST_F(SignatureTest, testUnsuccessfulEccVerificationWithWrongPublicKey)
{
    ...
    ASSERT_EQ(publicKey.getType(), AsymmetricKey::KeyTypes::ECC);
    AsymmetricPubkeyCtx ctxEccVerify1 = builder.eccMaskingFunction(DigestTypes::SHA1).build(publicKey);
    ASSERT_THROW(ctxEccVerify1.verify(_validEccSignatureSHA1,  _testMessageDigestSHA1),
                 MoCOCrWException);
}

TEST_F(SignatureTest, testInvalidCtx)
{
    std::vector<uint8_t> signature;
    AsymmetricPublicKey pubKey = *_keyPairRsa;
    ...
    EXPECT_THROW({
                     try {
                         AsymmetricPrivkeyCtx invalidCtxSign = builder.eccMaskingFunction(
                                 openssl::DigestTypes::SHA512).build(*_keyPairRsa);
                         invalidCtxSign.sign(_testMessageDigestSHA1);
    ...
    EXPECT_THROW({
                     try {
                         AsymmetricPubkeyCtx invalidCtxVerify = builder.eccMaskingFunction(
                                 openssl::DigestTypes::SHA512).build(pubKey);
                         invalidCtxVerify.verify(_validEccSignatureSHA1, _testMessageDigestSHA1);
                     } catch (const MoCOCrWException &e) {//this tests that it has the correct message
                         EXPECT_STREQ("When using a RSA key the RSAPadding context "
                                      "member needs to be set", e.what());
                         throw;
