...
INSTANTIATE_TEST_CASE_P(hmac,
                        HmacTests,
                        testing::ValuesIn(prepareTestDataForHmacTests()));

TEST_P(HmacTests, tests)
{
    testHmac(GetParam());
}
...
TEST(HmacTests3, checkConstructor)
{
    ...
    EXPECT_THROW(mococrw::HMAC(openssl::DigestTypes::SHA512, std::vector<uint8_t>()), MoCOCrWException);
}
