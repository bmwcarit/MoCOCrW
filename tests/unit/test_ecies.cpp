...
static const auto macFunc = [](const std::vector<uint8_t> &key) -> std::unique_ptr<MessageAuthenticationCode> {
...
public:
    void SetUp() override
    {
        ...
        auto cipherDecFunc = [zeroIV](const std::vector<uint8_t> &key) -> std::unique_ptr<SymmetricCipherI> {
            return AESCipherBuilder(mode, keySize, key).setIV(zeroIV).setPadding(padding).buildDecryptor();
        };

        auto cipherEncFunc = [zeroIV](const std::vector<uint8_t> &key) -> std::unique_ptr<SymmetricCipherI> {
