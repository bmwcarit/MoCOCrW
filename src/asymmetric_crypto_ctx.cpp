...
RSAEncryptionPrivateKeyCtx::RSAEncryptionPrivateKeyCtx(
        const AsymmetricPrivateKey &key, std::shared_ptr<RSAEncryptionPadding> padding)
...
RSAEncryptionPrivateKeyCtx &RSAEncryptionPrivateKeyCtx::operator=(
        const RSAEncryptionPrivateKeyCtx &other)
{
...
RSAEncryptionPublicKeyCtx &RSAEncryptionPublicKeyCtx::operator=(
        const RSAEncryptionPublicKeyCtx &other)
{
...
{
public:
    RSASignatureImpl(const Key &key,
    ...
    std::vector<uint8_t> signDigest(const std::vector<uint8_t> &messageDigest)
    {
        ...
        if (messageDigest.size() != expectedDigestSize) {
            throw MoCOCrWException(
                    (boost::format{"Expected digest of size %1%"} % expectedDigestSize).str());
        }
...
RSASignaturePrivateKeyCtx &RSASignaturePrivateKeyCtx::operator=(
        const RSASignaturePrivateKeyCtx &other)
{
...
std::vector<uint8_t> RSASignaturePrivateKeyCtx::signDigest(
...
ECDSASignaturePrivateKeyCtx::ECDSASignaturePrivateKeyCtx(const AsymmetricPrivateKey &key,
                                                         openssl::DigestTypes hashFunction)
        : _impl(std::make_unique<ECDSASignaturePrivateKeyCtx::Impl>(
                  key, hashFunction, ECDSASignatureFormat::ASN1_SEQUENCE_OF_INTS))
{
}
...
ECDSASignaturePrivateKeyCtx &ECDSASignaturePrivateKeyCtx::operator=(
        const ECDSASignaturePrivateKeyCtx &other)
{
...
std::vector<uint8_t> ECDSASignaturePrivateKeyCtx::signDigest(
        const std::vector<uint8_t> &messageDigest)
{
    return _impl->signDigest(messageDigest);
}
...
ECDSASignaturePublicKeyCtx::ECDSASignaturePublicKeyCtx(const AsymmetricPublicKey &key,
        ...
        : _impl(std::make_unique<ECDSASignaturePublicKeyCtx::Impl>(
                  key, hashFunction, SDAtoDSASignatureFormat(format)))
{
}
...
ECDSASignaturePublicKeyCtx::ECDSASignaturePublicKeyCtx(const X509Certificate &cert,
        ...
        : ECDSASignaturePublicKeyCtx(
                  cert.getPublicKey(), hashFunction, SDAtoDSASignatureFormat(format))
{
}
...
ECDSASignaturePublicKeyCtx &ECDSASignaturePublicKeyCtx::operator=(
        const ECDSASignaturePublicKeyCtx &other)
{
...
public:
    ...
    std::vector<uint8_t> signMessage(const std::vector<uint8_t> &message)
    {
        ...
        try {
            ...
            _EVP_DigestSignInit(
                    mctx.get(), DigestTypes::NONE, const_cast<EVP_PKEY *>(_key.internal()));
...
EdDSASignaturePrivateKeyCtx &EdDSASignaturePrivateKeyCtx::operator=(
        const EdDSASignaturePrivateKeyCtx &other)
{
    _impl = std::make_unique<EdDSASignaturePrivateKeyCtx::Impl>(*(other._impl));
    return *this;
}
...
public:
    ...
    void verifyMessage(const std::vector<uint8_t> &signature, const std::vector<uint8_t> &message)
    {
        try {
            ...
            _EVP_DigestVerify(
                    mctx.get(), signature.data(), signature.size(), message.data(), message.size());
        } catch (const OpenSSLException &e) {
            throw MoCOCrWException((boost::format{"Signature validation failed. "
                                                  "OpenSSL info: %1%"} %
                                    e.what())
...
EdDSASignaturePublicKeyCtx &EdDSASignaturePublicKeyCtx::operator=(
        const EdDSASignaturePublicKeyCtx &other)
{
    _impl = std::make_unique<EdDSASignaturePublicKeyCtx::Impl>(*(other._impl));
    return *this;
}
