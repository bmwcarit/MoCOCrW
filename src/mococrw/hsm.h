...
/**
 * An HSM Engine that leverages the PKCS11 interface.
 */
class Pkcs11Engine : public HsmEngine
{
public:
    Pkcs11Engine(const std::string &module_path, const std::string &pin);
    virtual ~Pkcs11Engine();

protected:
    /** The LibP11 context */
    p11::P11_PKCS11_CTX_Ptr pkcs11Ctx;
    p11::P11_SlotInfo slotInfo;
    ...
    /**
     * A helper function to prepare for an operation, such as key storage or generation,
     * on the HSM. It returns the slot, which the operation will be performed on.
     *
     * Internally, this function finds and authenticates with the first slot of an appropriate
     * token. A session will be open with both read and write permissions.
     */
    virtual p11::P11_PKCS11_SLOT_PTR startOperation();

    /**
     * A helper function to finish after an operation, such as key storage or generation,
     * on the HSM. It takes the pointer of the slot returned earlier by startOperation().
     *
     * Internally, the function will perform de-authentication with the slot.
     */
    virtual void finishOperation(p11::P11_PKCS11_SLOT_PTR token);

private:
    /** Engine ID for PKCS11 HSM. */
    static const std::string ENGINE_ID;
};
