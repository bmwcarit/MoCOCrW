...
/**
 * @brief This class supports encryption of plain texts using ECIES (as described in IEEE
 * 1363a-2004)
 *
 * This class supports encryption of plain texts using ECIES (as described in IEEE 1363a-2004) with
 * the following limitations. Supported:
 ...
 *
 * Elliptic curve integrated encryption scheme provides you the functionality to encrypt a plaintext
 * with a one time shared secret (based on an ephemeral key) using a symmetric encryption scheme.
 ...
 * 1. generate a random private public EC key pair (the public key is the mulitplication of the
 * private key with a generator: P_eph = r_eph * G)
 * 2. calculate the shared secret (P_shared = r_eph * P_recipient)
 * 3. generate the key for symmetric encryption and MAC (key_sym || key_mac = KDF(P_shared)) (using
 * the optional salt)
 * 4. encrypt the plaintext
 * 5. calculate the MAC over the ciphertext (with the optional salt concatenated to the ciphertext)
 *
 * Send P_eph, ciphertext and MAC to the recipient. This is left to the caller. This class doesn't
 * do any serialization as there is no agreed standard yet.
 *
 * For using the ECIESEncryptionCtx a builder class is provided which takes the parameter described
 * above as input. Afterwards the ECIESEncryptionCtx can be used like a symmetric cipher. This means
 * that you can call update for adding plaintext bytes to the cipertext and finish() for calculating
 * the MAC.
 *
 * The MAC and the ephemeral key can be obtained via getter functions.
 *
 * Workflow for Encryption:
 * 1. Get the necessary data (public key, MAC scheme, KDF scheme, symmetric cipher scheme, optional
 * salts)
 * 2. Construct an ECIESEncryptionCtx using the ECIESEncryptionCtxBuilder providing providing the
 * data from 1.
...
public:
     ...
     *
     * After invoking this function, the ephemeral key and the authentication key can be obtained
     ...
     *
     * The ephemeral key is generated in the constructor of the instance. This public key (in
     * combination with the private key of the receiver) is necessary for decrypting the ciphertext.
     ...
     *
     * The authentication tag is calculated as the last step during encryption. This tag verifies
 ...
 *
 * For decrypting you need the private key of the recipient instead of his/her/its public key and
 * the ephemeral key received from the sender.
 *
 * The algorithm for ECIES decryption is as follows:
 * 1. calculate the shared secret (P_shared = P_ephemeral * r_recipient = r_eph * r_recipient * G =
 * r_eph * P_recipient)
 * 2. generate the key for symmetric encryption and MAC (key_sym || key_mac = KDF(P_shared)) (with
 * the optional salt)
 * 3. calculate and verify the MAC (using the optional salt for MAC)
 * 4. decrypt the ciphertext
 *
 * For using the ECIESDecryptionCtx a builder class is provided which takes the parameter described
 * above as input. Afterwards the ECIESDecryptionCtx can be used like a symmetric cipher.
 *
 * The ephemeral key and the MAC can be set after creating the ECIESDecryptionCtx object. After
 * setting the ephemeral key the shared secret and the keys for decrypting and MAC are calculated
 * automatically.
 *
 * After setting the ephemeral key you can use update() for adding ciphertext bytes and finish() for
 * verifing the MAC and getting the plaintext. If the verification fails no plaintext is returned,
 * but an error is thrown instead.
 *
 * Workflow for Decryption:
 * 1. Receive the data
 * 2. Split it into ephemeral key, ciphertext and MAC
 * 3. Get the necessary data (private key, MAC scheme, KDF scheme, symmetric cipher scheme, optional
 * salts)
 * 4. Construct an ECIESDecryptionCtx using the ECIESDecryptionCtxBuilder providing the cipher data
 * (@see ECIESEncryptionCtx). The ephemeral key is needed here.
...
public:
     ...
     *
     * During verification the calculated authentication tag is compared to the given one received
     * via setMAC(). The authentication tag is calculated over the ciphertext provided via update().
     *
     ...
     *
     * As the last step during encryption the authentication tag is calculated. Set this tag before
     * calling finish() so that the authenticity the received message can be verified. If the MAC is
     * not set a MoCOCrWException is thrown when finish() is invoked.
 ...
 *
 * The ECIESCtxBuilder simplifies the creation of the ECIES Encryption and Decryption contexts. This
 * is done by providing setters for all available parameters according to IEEE 1363a and default
 * values for these parameters if none is set.
 */
class ECIESCtxBuilder
{
public:
     ...
     *
     * As the key for MAC is generated during ECIES key generation, the MAC object has to be
     instantiated after that.
     * This function is a dependency injection.
     * (example using HMAC-SHA512:
     \code{.cpp}
        auto macFunc = [](const std::vector<uint8_t> &key) ->
     std::unique_ptr<MessageAuthenticationCode> { return
     std::make_unique<mococrw::HMAC>(openssl::DigestTypes::SHA512, key);
        };
     \endcode
     * )
     * @param func A std::function which returns an object implementing the
     MessageAuthenticationCode interface
     * @return builder instance
     */
    ECIESCtxBuilder &setMacFactoryFunction(
     ...
     *
     * As the key for AES is generated during ECIES key generation, the AES object has to be
    instantiated after that.
     * This function is a dependency injection.
    ...
    \endcode
     * @param func A std::function which will be provided with the symmetric key (of key size
    provided
     * in setSymmetricCipherKeySize) and should return an object implementing the SymmetricCipherI
    interface.
     * @return builder instance
     */
    ECIESCtxBuilder &setSymmetricCipherFactoryFunction(
     ...
     * For decrypting the ciphertext the ephemeral public key, which was generated during
